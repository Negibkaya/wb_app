...
const ROBOTO_FONT_URL = 'https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.66/fonts/Roboto/Roboto-Regular.ttf';
...
async function ensureCyrillicFont(doc: jsPDF): Promise<string> {
  try {
    ...
    console.warn('Failed to load Roboto font for Cyrillic. Falling back to default system font.', error);
      ...
      try {
        ...
        for (const row of rawRows) {
          ...
          for (const key of Object.keys(row)) {
            ...
            if (cleanKey === 'название' || cleanKey === 'name' || cleanKey === 'товар' || cleanKey === 'наименование') {
              name = value;
            } else if (cleanKey === 'баркод' || cleanKey === 'barcode' || cleanKey === 'штрихкод') {
              barcode = value;
            } else if (cleanKey === 'артикул' || cleanKey === 'article' || cleanKey === 'sku') {
              article = value;
            }
          }
        ...
        if (products.length === 0) {
          throw new Error('Не найдено корректных данных. Убедитесь, что в файле есть столбцы: Название, Баркод, Артикул.');
        }

        resolve(products);
      } catch (err) {
        reject(err instanceof Error ? err : new Error('Ошибка при разборе файла Excel/CSV'));
      }
    };
 ...
 * The file consists of two columns in chronological order of addition:
 * Column 1: "Баркод"
 * Column 2: "Количество"
 */
export function exportToXLSX(items: ShipmentItem[], fileName = 'shipment.xlsx'): void {
 ...
 */
export async function exportToPDF(items: ShipmentItem[], shipmentName = 'Поставка WB'): Promise<void> {
  const doc = new jsPDF({
    orientation: 'p',
    unit: 'mm',
    format: 'a4',
  ...
  sortedBoxNumbers.forEach((boxNum, index) => {
    ...
    const sortedBoxItems = [...boxItems].sort((a, b) => a.createdAt - b.createdAt);

    // Stats for the box
    const uniqueBarcodes = new Set(sortedBoxItems.map(item => item.product.barcode)).size;
    const totalQuantity = sortedBoxItems.reduce((acc, item) => acc + item.quantity, 0);
